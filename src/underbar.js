(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection,function(value, index, collection){
    	newCollection.push(iterator(value, index, collection))
    })
  ...
  _.shuffle = function(array) {
    var origArray = Array.prototype.slice.call(array);
    var newArray = [];
    while(newArray.length<origArray.length){
      var index = Math.floor(Math.random() * origArray.length);
      if(!_.contains(newArray, origArray[index])){
  ...
  _.sortBy = function(collection, iterator) {
    if(typeof iterator === 'string'){
      return collection.sort(function(a, b){
        var itemA = a[iterator];
        var itemB = b[iterator];
        if(itemA<itemB){
          return -1;
        }
        if(itemA>itemB){
      ...
      return collection.sort(function(a, b){
        var itemA = iterator(a);
        var itemB = iterator(b);
        if(itemA<itemB){
          return -1;
        }
        if(itemA>itemB){
